...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='This script will deploy VPC traffic mirroring infrastructure into an AWS account to begin monitoring network traffic within that VPC. The traffic will be mirrored to an EC2 instance that is created and that EC2 instance will exfiltrate the traffic to an S3 bucket that you specify.')
